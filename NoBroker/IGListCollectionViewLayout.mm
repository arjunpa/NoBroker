...
- (void)cacheLayout {
    ...
    for (NSInteger section = 0; section < sectionCount; section++) {
        ...
        for (NSInteger item = 0; item < itemCount; item++) {
            ...
            NSIndexPath *indexPath = [NSIndexPath indexPathForItem:item inSection:section];
            if(item == 0){
                if ([(id)delegate respondsToSelector:@selector(layout:insetXForFirstIndexPathInNewLine:)]){
                    
                    itemX += [(id)delegate layout:self insetXForFirstIndexPathInNewLine:indexPath];
                }
            }
